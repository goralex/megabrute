...
### Usage of ./megabrute:
  ...
  -f string
        Path to the file with target urls. Or use nmap:path.xml or masscan:path.xml to get targets from nmap or masscan.
        
  -n int
        Set number of goroutines functions (number of parallel requests). Default = 10.
        
  -o string
        Sets file for output. Default is /tmp/megabrute-resalt.txt.
  ...
  -r int
        Sets deep of recursion. Default = 1.
        
  -t string
        Type of url: IIS, Apache, ect. Default: "Dir".
        
  -u string
        Sets target url.
        
  -tmp string
        Sets temporary file to store intermediate values. By default they stores in memory.
...
#### Structure of output:
Output components: url,md5,ssdeep hash,responds header, body, payload.
#### jq parsing examples:
